...
public class DepartmentController extends HttpServlet {
    ...
    /**
     * POST, 49.234.209.55：//localhost:8080/department.ctl, 增加学院
    ...
    /**
     * DELETE, 49.234.209.55：//localhost:8080/department.ctl?id=1, 删除id=1的学院
    ...
    /**
     * PUT, 49.234.209.55：//localhost:8080/department.ctl, 修改学院
     *
    ...
    /**
     * GET, 49.234.209.55：//localhost:8080/department.ctl?id=1, 查询id=1的学院
     * GET, 49.234.209.55：//localhost:8080/department.ctl, 查询所有的学院
     * 把一个或所有学院对象响应到前端
     * @param request
     * @param response
     * @throws ServletException
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        //声明一个string类型的变量，读取参数id
        String id_str = request.getParameter("id");
        String paraType_str = request.getParameter("paraType");
        //创建JSON对象message，以便往、向前端响应信息
        JSONObject message = new JSONObject();
        try {
            //如果参数id或paraType不为空
            if (id_str != null || paraType_str != null){
                ...
                if (paraType_str != null){
                    //如果paraType获取的参数值为school
                    if (paraType_str.equals("school")){
                        //将str类型的转化为int类型
                        int id = Integer.parseInt(id_str);
                        //执行responseDepartmentSchool方法
